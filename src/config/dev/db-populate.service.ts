...
export class DbPopulateService implements OnApplicationBootstrap {
    readonly #tabellen = ['kategorie', 'fragebogen', 'frage'];
    ...
    async #populatePostgres() {
        ...
        await dataSource.query(`SET search_path TO questionnaire_schema;`);
        const copyStmt =
            "COPY %TABELLE% FROM '/csv/questionnaire/%TABELLE%.csv' (FORMAT csv, DELIMITER ';', HEADER true);";
