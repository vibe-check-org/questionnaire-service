...
import { entities } from '../fragebogen/model/entity/entities.js';

const { db } = config;

// nullish coalescing
const database =
    (db?.database as string | undefined) ?? Fragebogen.name.toLowerCase();

const host = db?.host as string | undefined;
const username =
    (db?.username as string | undefined) ?? Fragebogen.name.toLowerCase();
